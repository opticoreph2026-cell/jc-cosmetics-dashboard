...
export default function APPage() {
  ...
  async function handlePayment(id: string) {
    ...
    try {
      ...
      setPaying(null);
      setPayAmount("");
      const d = await fetch(`/api/ap?status=${status}`).then((r) => r.json());
  ...
  return (
    <div className="space-y-6">
      <h1 className="font-display text-2xl text-jc-anchor">Accounts Payable</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-sm border border-jc-blush bg-white p-4">
          <p className="text-xs uppercase tracking-wider text-jc-anchor/60">Total AP</p>
      ...
      <div className="overflow-x-auto rounded-sm border border-jc-blush">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-jc-blush">
            {data.length === 0 ? (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-xs text-jc-anchor/40">No AP items.</td></tr>
            ) : data.map((item) => {
              ...
              return (
                <tr key={item.id} className={`hover:bg-jc-cream/20 ${item.status === "UNPAID" ? "bg-red-50/50" : item.status === "PARTIAL" ? "bg-amber-50/50" : ""}`}>
                  <td className="px-4 py-3">
                    <Link href={`/suppliers/${item.supplierId}`} className="text-jc-rose-gold hover:underline">{item.supplier.name}</Link>
                  </td>
